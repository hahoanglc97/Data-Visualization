...
def fetch_data(url):
    try:
      ...
      WebDriverWait(driver, 15).until(EC.presence_of_element_located((By.CSS_SELECTOR, 'div.X0xUb5'))) # Wait until `price` loaded within 10 secs or timeout
      driver.execute_script("window.scrollTo(0, document.body.scrollHeight) * 0.5;")
      WebDriverWait(driver, 15).until(EC.presence_of_element_located((By.CSS_SELECTOR, 'div.product-ratings'))) # Wait until `price` loaded within 10 secs or timeout
...
links = pd.read_csv(const.DATASET_FOLDER + const.CLASS_FILE[1])['link']
...
outdir = f'{const.EXTRACTED_FOLDER}do_dien_tu/'
...
for i in range(0,19):
  ...
  df.to_csv(fullname, encoding='utf-8')
  item_count += len(name)
  name, price = [], []
  rating, customer_rating = [], []
  noOfSold, brand, stock, categories = [], [], [], []
...
print(f"Fetched {item_count} out of {link_count} links.")
print(f"Failed to fetch {link_count - item_count} links due to exception/timeout.")



df_timeout = pd.DataFrame(links_timeout, columns=['link'])
df_timeout.to_csv(f'{const.EXTRACTED_FOLDER}timeout_2.csv')