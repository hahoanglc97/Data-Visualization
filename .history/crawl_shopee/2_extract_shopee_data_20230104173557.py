...
def fetch_data(url):
    try:
      ...
      WebDriverWait(browser, 30).until(EC.presence_of_element_located((By.CSS_SELECTOR, 'div.product-ratings'))) # Wait until `shop result` loaded within 10 secs or timeout
      ...
      five_start_value = rating_details[2]
      four_start_value = rating_details[3]
      three_start_value = rating_details[4]
      two_start_value = rating_details[5]
      one_start_value = rating_details[6]
      comments_value = rating_details[7]
      image_video_comments_value = rating_details[8]
...
# links = pd.read_csv(const.DATASET_FOLDER + const.CLASS_FILE[0])
...
df.to_csv(f'{const.EXTRACTED_FOLDER}result.csv')