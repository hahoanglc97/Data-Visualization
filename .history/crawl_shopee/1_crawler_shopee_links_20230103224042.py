...
def load_links(base_url):
  ...
  for key in const.KEYWORDS_SEARCH:
    ...
    for keyword in const.KEYWORDS_SEARCH[key]:
      for page in range(0,const.MAX_PAGES):
          try:
              ...
              WebDriverWait(browser, 60).until(EC.presence_of_element_located((By.CSS_SELECTOR, 'div.shopee-search-item-result__items'))) # Wait until `shop result` loaded within 10 secs or timeout
    ...
    pd.DataFrame(links, columns=['link']).to_csv(f'./crawl_shopee/data/links_product_{key}.csv')
